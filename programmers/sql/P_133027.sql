...
chocolate
*/
WITH FH AS -- 상반기 맛 별 주문량 
    (SELECT FLAVOR, SUM(TOTAL_ORDER) AS TOTAL_ORDER_F
    FROM FIRST_HALF
    GROUP BY FLAVOR),
JUL AS -- 7월 맛 별 주문
...
SELECT FLAVOR
FROM(
SELECT F.FLAVOR, SUM(J.TOTAL_ORDER_J) + SUM(F.TOTAL_ORDER_F) AS TOTAL_ORDER
FROM FH F 
INNER JOIN JUL J ON F.FLAVOR = J.FLAVOR
GROUP BY F.FLAVOR
